...
def count(founded_classes,im0):
  model_values=[]
  aligns=im0.shape
  align_bottom=aligns[0]-100
  align_right=(aligns[1]/1.7 ) 

  for i, (k, v) in enumerate(founded_classes.items()):
    a=f"{k} = {v}"
    model_values.append(k)
    model_values.append(v)
    align_bottom=align_bottom-35                                                   
    cv2.putText(im0, str(a+" found") ,(int(align_right),align_bottom), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,0,255),2,cv2.LINE_AA)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', default='/content/drive/MyDrive/yolo stock market videos', help='save results to project/name')
